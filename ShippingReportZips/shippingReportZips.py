...
class App(QWidget):
  ...
  def __init__(self):
    super().__init__()
    self.title = 'Extract Shipping Report Zip Codes'
    self.left = 10
    self.top = 10
    self.width = 320
    self.height = 200
  ...
  def on_click(self):
    ...
    #  with open(filename, 'r') as f:  # open in readonly mode
      excel_data = pd.read_excel(filename, usecols='G')
    ...
    report = 'AllShippingReportZips-' + datetime.now().strftime('%m_%d_%Y') + '.xlsx'
